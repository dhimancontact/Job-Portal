...
class AuthController extends BaseController {
     ...
     * @var array
     */
    public $freeAccessActions = ['login', 'registration', 'logout', 'confirm-registration-email'];
    ...
    public function actionRegistration() {
        ...
        $model = new $module->registrationFormClass;
        ...
        if ($model->load(Yii::$app->request->post()) AND $model->validate()) {
            $model->resume = \yii\web\UploadedFile::getInstance($model, 'resume');
            if($model->resume)
                $filename=rand().$model->resume->baseName . '.' . $model->resume->extension;
            $model->resume->saveAs('uploads/' . $filename);
            $model->resume=Url::to('uploads/' . $filename, true);
                ...
                if ($this->triggerModuleEvent(UserAuthEvent::AFTER_REGISTRATION, ['model' => $model, 'user' => $user])) {
                    if ($user) {
                        if (Yii::$app->getModule('user-management')->useEmailAsLogin AND Yii::$app->getModule('user-management')->emailConfirmationRequired) {
                            ...
                            $roles = (array) $module->rolesAfterRegistration;
