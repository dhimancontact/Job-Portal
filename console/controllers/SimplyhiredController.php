<?php
/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
namespace console\controllers;
 
use yii\console\Controller;
use console\models\Job;
 
class SimplyhiredController extends Controller {
    public function actionIndex() {
        $hour=date("H");
        $keywords = (new \yii\db\Query())
            ->select(['k.id', 'k.keyword', 'k.location', 'k.full_scrapped'])
            ->from('search_keys k')
            ->leftJoin('key_priority p', 'p.search_key_id=k.id')
            ->where(['p.run_hour' => $hour])
            ->orderBy('p.priority ASC')
            ->all();
        $indeedUrl="http://www.simplyhired.com/search?";
        foreach($keywords as $keyword){
            $url='';
            if($keyword['location']){
                $url=$indeedUrl.'q='.$keyword['keyword'].'&l='.$keyword['location'].'&sb=dd';
            }else{
                $url=$indeedUrl.'q='.$keyword['keyword'].'&sb=dd';
            }
            $content=$this->getContent($url.'&pn=1');
            if(!$keyword['full_scrapped']){
                $total_count=$this->getCount($content);
                $keyword_id=$keyword['id'];
                \Yii::$app->db->createCommand()->update('search_keys', ['full_scrapped' => 1], "id = $keyword_id")->execute();
            }else{
                $total_count=50;
            }
            $list=$this->getList($content, $url, $total_count, 1, $keyword['keyword'], $keyword['location']);
            //echo $list." items processed";
        }
    }

    public function getContent($link)
    {
        $ch = curl_init($link);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        $content=curl_exec($ch);
        if($content === false)
        {
            echo "Waittt:::".$content;
            echo 'Curl error: ' . curl_error($ch);
            flush();
            return(false);

        }
        return($content);
    }

    public function getCount($content){
        $dom = new \DOMDocument;
        @$dom->loadHTML($content);
        $finder = new \DomXPath($dom);
        $countNode= $finder->query("//div[@class='result-text']");
        $count=0;
        if(isset($countNode->item(0)->nodeValue) && $countNode->item(0)->nodeValue){
            $keys=explode('of ', $countNode->item(0)->nodeValue);
            $count=trim(array_pop($keys));
        }
        $count=str_replace(',', '', $count);
        return (int)$count;
    }

    public function getList($content, $url, $total_count, $count, $searchKey, $searchLocation){
        $simplyDetailUrl="http://www.simplyhired.com";
        $dom = new \DOMDocument;
        @$dom->loadHTML($content);
        $finder = new \DomXPath($dom);
    
        $list_items= $finder->query("//div[contains(@class, 'card') and contains(@class, 'js-job')]");
        $jobs=[];
        
        foreach($list_items as $list_item){
            $id='';
            $job=[];
            
            $title=$list_item->getElementsByTagName("a");
            $correct=0;
            foreach($title as $til){
                if($til->getAttribute('class')=="card-link js-job-link"){
                    $linkUrl=$til->getAttribute('href');
                    $parts = parse_url($linkUrl);
                    parse_str($parts['query'], $query);
                    if(isset($query['a'])){
                        $id=$query['a'];
                    }
                    $job['link']=$simplyDetailUrl.$linkUrl;
                    $correct=1;
                }
            }
            if(!$correct){
                continue;
            }

            $heading=$list_item->getElementsByTagName("h2");
        
            foreach($heading as $head){
                if($head->getAttribute('class')=="serp-title" && $head->getAttribute('itemprop')=="title"){	
                    $job['title']=$head->nodeValue;
                }
            }
            if(!isset($job['title'])){
                $job['title']=$heading->item(0)->nodeValue;
            }
            if(!isset($job['title'])){
                continue;
            }
            if($id){
                if((new \yii\db\Query())->from('job')->where(['site_job_id' => $id])->exists()){
                    continue;
                }
            }

            $details=$list_item->getElementsByTagName("span");
            foreach($details as $detail){
                if(($detail->getAttribute('class')=="serp-company" || $detail->getAttribute('class')=="jobposting-company") && $detail->getAttribute('itemprop')=="name"){
                    $job['company']=trim(strip_tags($detail->nodeValue));
                }
                if(($detail->getAttribute('class')=="serp-location" || $detail->getAttribute('class')=="jobposting-location") && $detail->getAttribute('itemprop')=="addressLocality"){
                    $job['location']=$detail->nodeValue;
                }

                if($detail->getAttribute('class')=="serp-timestamp" || $detail->getAttribute('class')=="jobposting-timestamp"){
                    $posted_date=$detail->nodeValue;
                    if(trim($posted_date) && strpos($posted_date, ' ago') !== false){
                        if (strpos($posted_date, '30+') !== false) {
                            $posted_date='31 days';
                        }else{
                            $posted_date=explode(' ago',$posted_date);
                            $posted_date=$posted_date[0];
                        }
                        $date = (new \DateTime())->modify('-'.$posted_date);
                        $job['posted_date']=$date->format('Y-m-d H:i:s');
                    }else{
                        $job['posted_date']=NULL;
                    }
                }
            }

            $descriptions=$list_item->getElementsByTagName("p");
            foreach($descriptions as $description){
                if(($description->getAttribute('class')=="serp-snippet" || $description->getAttribute('class')=="jobposting-snippet") && $description->getAttribute('itemprop')=="description"){
                    $job['short_description']=$description->nodeValue;
                    $job['description']=$job['short_description'];
                }
            }

            $job_title=$job['title'];
            $job_description=isset($job['description'])?$job['description']:'';
            $job_short_description=isset($job['short_description'])?$job['short_description']:'';
            if(isset($job['location'])){
                if (strpos($job['location'], ',') !== false) {
                    $loca=explode(',',$job['location']);
                    $job_city=trim($loca[0]);
                    $state_text=trim(isset($loca[1])?$loca[1]:'');
                    if (strpos($state_text, ' ') !== false) {
                        $state_text=substr($state_text,0,2);
                    }
                    $job_state=$state_text;
                }else{
                    $job_city=isset($job['location'])?$job['location']:'';
                    $job_state='';
                }
            }else{
                $job_city=$searchLocation;
                $job_state='';
            }
            $job_company=isset($job['company'])?$job['company']:'';
            $job_job_site='simplyhired';
            $job_site_job_id=$id;
            $job_link=isset($job['link'])?$job['link']:'';
            $job_posted=isset($job['posted_date'])?$job['posted_date']:'';
            
            $location_id=null;
            $company_id=null;

            if($job_company){
                $company=(new \yii\db\Query())->from('company')->where(['company_name' => $job_company])->orWhere(['subdomain'=>$job_company])->one();
                if(!$company){
                    \Yii::$app->db->createCommand()
                        ->insert('company', [
                        'company_name' => $job_company
                        ])->execute();
                    $company_id=\Yii::$app->db->getLastInsertID();
                }else{
                    $company_id=$company['id'];
                }
            }

            if($job_city || $job_state){
                $query=(new \yii\db\Query())->from('location');
                if($job_city && $job_state){
                    $location=$query->where(['location' => $job_city])->andWhere(['state'=>$job_state])->one();
                    if(!$location){
                        \Yii::$app->db->createCommand()
                            ->insert('location', [
                            'location' => $job_city,
                            'state' => $job_state
                            ])->execute();
                        $location_id=\Yii::$app->db->getLastInsertID();
                    }else{
                        $location_id=$location['id'];
                    }
                }else if($job_city){
                    $location=$query->where(['location' => $job_city])->orWhere(['state' => $job_city])->one();
                    if(!$location){
                        \Yii::$app->db->createCommand()
                            ->insert('location', [
                            'location' => $job_city,
                            'state' => $job_city
                            ])->execute();
                        $location_id=\Yii::$app->db->getLastInsertID();
                    }else{
                        $location_id=$location['id'];
                    }
                }else {
                    $location=$query->where(['state' => $job_state])->orWhere(['location' => $job_state])->one();
                    if(!$location){
                        \Yii::$app->db->createCommand()
                            ->insert('location', [
                            'location' => $job_state,
                            'state' => $job_state
                            ])->execute();
                        $location_id=\Yii::$app->db->getLastInsertID();
                    }else{
                        $location_id=$location['id'];
                    }
                }
            }

            $jobModel=new Job;
            $jobModel->company_id=$company_id;
            $jobModel->job_title=$job_title;
            $jobModel->job_description=$job_description;
            $jobModel->short_description=$job_short_description;
            $jobModel->skill_set=$searchKey;
            $jobModel->speciality=$searchKey;
            $jobModel->location_id=$location_id;
            $jobModel->site_job_id=$id;
            $jobModel->job_posted=$job_posted;
            $jobModel->save();
        }
        $count=$count+1;
        if(($count*10)<($total_count+10) && $count < 100){
            $content=$this->getContent($url.'&pn='.$count);
            $list=$this->getList($content, $url, $total_count, $count, $searchKey, $searchLocation);
        }
        return $count;
    }


    public function getDetail($link){
        $content=$this->getContent($link);
        $dom = new \DOMDocument;
        @$dom->loadHTML($content);
        $finder = new \DomXPath($dom);
        $summaryNode= $finder->query("//span[@id='job_summary']");
        return $summaryNode->item(0)->nodeValue;
    }
}
